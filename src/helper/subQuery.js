...
export default {
  ...
  productAttributes(userId, currencyRate, tokenData) {
    const data = [
      ...
      [
        Sequelize.literal(`IFNULL((SELECT TRUNCATE(Product.price - ((discounts.discount_percent/100) * Product.price), 2) from discounts JOIN product_discounts ON product_discounts.discount_id = discounts.id WHERE discounts.status = 'active' AND product_discounts.status = 'active' AND product_discounts.product_id = Product.id), 0)   
    `),
        'discountPrice',
      ],
      [
  ...
  popularProduct() {
    return Sequelize.literal(
      '(Select count(order_products.product_id) from order_products join orders on orders.id = order_products.order_id where order_products.status != "deleted" and orders.status = "completed")',
    );
  },
